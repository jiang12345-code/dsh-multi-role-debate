...
## 🔐 远程访问权限控制（2026-09-02 v2.1 · M0 后暂停，等身份方案拍板）
- 需求：**具名访客的工作区共享**——"给具体某个人只看我某几个工作区，设置面板便捷配置；我的工作区就是我的秘密"。分层已确认：个人网站（www.jiangsan.vip，CF Pages）公开零拦截；DSH（harness.jiangsan.vip）登录后按 ACL 控权。
- **M0 实测结论**：① alpha.3 无官方多身份接缝，webServer 包装方案技术可行（`dsh-workspace-share\` v0.1.0 身份+ACL+管理 API 已写好并验证，v0.2.0 强制层代码在库）；② **CF Access 的 API 建 org 路径不可用**——新 org 无登录方式（OTP 无法经 API 启用），实测锁死本机+手机；③ **v0.2.0 强制层无 owner break-glass**，CF 删除后连 owner 手机都被自己插件 401。两教训已进 LESSONS 2026-09-02 条。
- **已回滚**：CF Access 应用已删、插件已从 bundles 摘除（16:47 重启后公网 401 指纹=原生文案确认），本机+手机访问已恢复（原生 token 门禁+30 天 cookie）。零 CF 依赖。
- **身份方案已拍板（2026-09-02 晚）= 插件自建账号体系**：注册（用户名+邮箱+密码）→ owner 面板审批 → 按工作区授权+档位；scrypt 哈希+登录限速+HMAC 会话 cookie（密钥持久化凭据库）；break-glass 三件套（loopback 恒 owner / meta.enforce 默认关 / owner 密码账号）；v1 仍 viewer-only（member 执行档=v1.1 工具围困）。CF 路径正式放弃。
- 实施 SSOT：`workspace-share-plan.md`（v2.2）+ Mnemon 文档 c36dec03。不变量 I1-I10 与 v1=viewer-only 结论不变。重要 API 事实：`/api` 每请求过 `requestRejection`（围栏+cookie），`rpc.intercept` 槽被 apiProxy 占，包 `connection.requestRejection/authorizeIndex` 两方法即可放行自定义身份；webServer 的 exact/prefixes/upgrades/fallback 均运行期可包。
- 开工时：先加载 `dsh-plugin-development` 技能；**bundle 插件拿 webServer 必须 `inject:['webServer']` 硬依赖 + ctx.webServer（inject 空+ctx.get 静默 404，v0.0.1 实测）**；重启守 `--trusted-host`/勿杀 cloudflared；.ps1 带 BOM。
